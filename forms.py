...
class RecordForm(FlaskForm):
    ...
    title = StringField('機種・レース名', validators=[Optional(),Length(max=100, message='100文字以内で入力してください')],render_kw={"placeholder":"(例)日本ダービー"})
...
class PlaceForm(FlaskForm):
    name = StringField('店舗名・開催場名', validators=[DataRequired('名前を入力してください'),Length(max=50, message='50文字以内で入力してください')],render_kw={"placeholder":"(例)東京競馬場"})
    category_id = SelectField('カテゴリ', coerce=int,validators=[DataRequired('カテゴリを選択してください')])
    submit = SubmitField('登録')
    
class CategoryForm(FlaskForm):
    name = StringField('カテゴリ名', validators=[DataRequired('名前を入力してください'),Length(max=50, message='50文字以内で入力してください')],render_kw={"placeholder":"(例)競馬"})
    submit = SubmitField('登録')